...
## Pipeline
For the pipeline. I took the raw text which were reviews and made them uniform by lowering case, punctuting and then tokenizing them to split the sentences into individual words. Considering our data set high ,every word matters hence the need to negate some words that would otherwise be removed, e.g. not by the stop words remover and lemmatized whatever remained.  Outputs were saved in a csv file for trasmitabilty.
...
| **Decision Tree** | BoW | 1.00 | 1.00 | **Overfitting**: Unpruned tree till pure. |
...
Models Findings.
For our models, we used a NB as a baseline and compared it to an unprunned Decision tree which ran until pure and finally the most effective of all, the Logistic regression. All models performed significantly well on our small dataset templates as expected with an accurracy of 100%. However, Being a sentiment analysis project, the model was splitting templates rather than finding sentiments which are crucual for this analysis. Having run a  two step model, one for raw counts and the other for weighted vectors. The Baseline, It did well because it's great at counting probabilities. Decision Tree; This one was fascinating but we had to keep an open eye. It achieved 100% accuracy, but the tree plot proved it was just memorizing the exact words in my 10 templates. It splits on the word money, the next split is buying and pictures. gini of 0.0 meaning it found a perfect split. Logistic regression; had positive classes being flagged as sentiments although randomly classified words which are not sentiments e.g. family. On  negative classes, like was used as a negative sentiment, which on real data could be positive.
